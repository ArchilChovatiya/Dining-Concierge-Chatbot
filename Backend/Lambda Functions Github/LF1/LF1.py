...
def send_recommendations(intent_request):
    ...
    response = client.send_message(
        QueueUrl='https://sqs.us-east-1.amazonaws.com/760023046737/Q1',
        MessageAttributes={
                    'cuisine': {
                        'DataType': 'String',
                        'StringValue': cuisine
...
def validate_user_inputs(location,cuisine,people,date,time,phoneno,email):
    ...
    if location is not None and location.lower() not in cities:
        return build_validation_result(False,
                                       'Location',
                                       'That is a beautiful place. Unfortunately, we only suggest restaurants in the Manhattan area in New York City. Currently, we are not covering {} area but will look forward to it!'.format(location))
    ...
    if date is not None:
        if not isvalid_date(date):
            ...
            return build_validation_result(False, 'Date', 'You cannot choose a date from the past (Time travel doesn\'t exists.). Choose today or future dates')

    if cuisine is not None and cuisine.lower() not in valid_cuisines :
        return build_validation_result(False, 'Cuisine','We have Italian, American, Japanese, Indian, Chinese and Mexican cusines. Sounds delicious, right? Though we will add more options soon')
    ...
    if phoneno is not None:
        if not isvalid_phone(phoneno):
            return build_validation_result(False,'PhoneNo','PhoneNo. can not be greater than 10')
